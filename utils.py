...
def showMenu():
    ...
    print(f'{Style.BRIGHT}{Fore.YELLOW}4: Format changer [Email:Pass:Token] => [Token] {Style.RESET_ALL}')
    ...
    print(f'{Style.BRIGHT}{Fore.YELLOW}7: Username Changer (Username list required(input/usernames.txt)) [Email:Pass:Token] (Soon){Style.RESET_ALL}')
...
def getInviteInfo(rawInvite):
    with Client(proxies=f"http://{choice(open('input/proxies.txt').read().splitlines())}") as client:
        res = client.get(f'https://discord.com/api/v9/invites/{rawInvite}?with_counts=true', headers={"Authorization": "undefined"}).json()
        return res
def getGoodToken():
